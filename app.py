...
def v3_trades_list():
    ...
    # One campaign per (symbol, entry timestamp): all trims/scale-outs of the same
    # entry collapse into a single row regardless of how many exits it had.
    from collections import defaultdict
    grouped = defaultdict(list)

    def _group_key(t):
        symbol = t["symbol"]
        entry_dt = t.get("entry_datetime") or t.get("entry_date")
        if entry_dt:
            return (symbol, "entry", entry_dt)
        return (symbol, "exit", t.get("exit_date"))
    ...
    for group in grouped.values():
        if len(group) == 1:
            ...
            t["ticker"] = t.get("symbol")
            campaign_token = t.get("entry_datetime") or t.get("entry_date") or ""
