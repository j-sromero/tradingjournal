...
def main() -> None:
    ...
    parser.add_argument("--token", default=os.getenv("FLEX_TOKEN", ""), help="IBKR Flex token")
    parser.add_argument("--query-id", default=os.getenv("FLEX_QUERY_ID", ""), help="IBKR Flex query id")
    parser.add_argument("--html-file", default="", help="Alternative source: IBKR HTML statement path")
    parser.add_argument("--db", default="journal_v3.db", help="SQLite DB path")
    args = parser.parse_args()

    init_db(args.db)
    html_file = (args.html_file or "").strip()
    if html_file:
        rows = parse_trades_html(html_file)
    else:
